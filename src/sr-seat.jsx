...
function SeatSR(){
    ...
    useEffect(()=>{
        const fetchD = async()=>{
            try {
                const d = await axios.get("https://api-sc-pgsn.onrender.com/seatSRget");
    ...
    return(
        ...
        {data && data.length > 0 ? (
                    data.map((ele, index) => (
                        <div key={index} className="card" style={{ width: "18rem" }} onClick={e=>window.location.href=`/sr?sku=${ele.Sku}`}>
                        <img
